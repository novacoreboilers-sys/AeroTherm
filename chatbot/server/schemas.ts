...
export const leadSubmissionSchema = z.object({
  ...
  answers: z.record(z.string().max(80), z.string().trim().min(1).max(300)),
...
export function normalizeAnswer(question: ChatQuestion, rawValue: string) {
  ...
  switch (question.type) {
    ...
    case "text": {
      const text = question.id === chatbotConfig.fieldIds.name
      ...
      if (question.id === chatbotConfig.fieldIds.name) {
        ...
        if (/\b(?:website|web site|email|phone|service|business|help|booking|bookings|software|marketing|automation|nothing|unknown|anonymous|test|user)\b/i.test(text)) return null;
      }
