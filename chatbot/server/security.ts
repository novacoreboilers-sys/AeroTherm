...
const globalRateStore = globalThis as typeof globalThis & { __aerothermChatRates?: Map<string, RateEntry> };

export function checkMemoryRateLimit(fingerprint: string, limit = 15, windowMs = 60_000) {
  const store = globalRateStore.__aerothermChatRates ??= new Map<string, RateEntry>();
  const now = Date.now();
