...
export async function sendClientConfirmation(lead: LeadRecord) {
  ...
  await sendResendEmail({
    from,
    to: lead.answers[chatbotConfig.fieldIds.email],
    subject: chatbotConfig.email.clientSubject,
    html: content.html,
    text: content.text,
...
export async function sendTeamNotification(lead: LeadRecord) {
  ...
  await sendResendEmail({
    from,
    to,
    replyTo: lead.answers[chatbotConfig.fieldIds.email],
    subject: `New ${lead.qualification === "qualified" ? "qualified " : ""}lead: ${lead.answers[chatbotConfig.fieldIds.name]}`,
    html: content.html,
    text: content.text,
