...
import { syncLeadToGoogleSheets } from "./google-sheets";
...
async function runDelivery(
  lead: LeadRecord,
  channel: "googleSheets" | "clientEmail" | "teamEmail",
  task: () => Promise<void>,
) {
  try {
    await task();
...
export async function deliverLead(lead: LeadRecord) {
  ...
  if (lead.googleSheetsStatus !== "sent") {
    deliveries.push(runDelivery(lead, "googleSheets", () => syncLeadToGoogleSheets(lead)));
  }
